...
import 'package:notes_app/screens/watch_note_screen.dart';
import 'package:notes_app/utils/assets_constants.dart';
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('StickyPal'),
      actions: [
        buildActionIcon(
          iconPath: AssetsConstants.icDustbin,
          onTap: () async {
        ...
        buildActionIcon(
          iconPath: AssetsConstants.icSearch,
          onTap: () {
            Navigator.push(
              context,
              MaterialPageRoute(
  ...
  Widget _buildListOrEmpty() {
    if (_noteList.isEmpty) {
      return buildEmptyNotesUi(_size, path: AssetsConstants.svgEmptyNotes);
    } else {
  ...
  ConfirmDismissCallback deleteNoteConfirmDismiss(int index) {
    return (direction) async {
      _deleteListItem = await showDeleteNoteDialog(context);
      if (_deleteListItem) {
        context.read<NotesBloc>().add(DeleteNoteEvent(_noteList[index].id!));
        _deleteListItem = false;
